...
bolt_app = App(
    ...
    # Skip auth.test on init — speeds up startup and lets tests run with fake tokens
...
def _blocks_for(poll, counts, voters):
    return render_poll_blocks(
        poll.id, poll.question, poll.options, counts, voters, poll.creator_id,
        anonymous=poll.anonymous, multi_choice=poll.multi_choice,
        expires_at=poll.expires_at, closed=poll.closed,
    )
...
def _post_poll(client, channel_id, creator_id, question, options, **kwargs):
    ...
    resp = client.chat_postMessage(
        channel=channel_id, blocks=_blocks_for(poll, counts, voters), text=f"Poll: {question}",
    )
...
def _refresh_poll_message(client, poll_id):
    ...
    client.chat_update(
        channel=poll.channel_id, ts=poll.message_ts,
        blocks=_blocks_for(poll, counts, voters), text=f"Poll: {poll.question}",
    )


def _parse_flags(tokens: list[str]) -> tuple[dict, list[str]]:
    """Pull `--multi`, `--anon`, `--expires=30m` flags out of the token list."""
...
def handle_poll_command(ack, command, client, respond):
    ...
    try:
        ...
        return respond('⚠️ Parse error. Try `/poll "Q?" "A" "B" [--multi] [--anon] [--expires=30m]`')

    flags, parts = _parse_flags(tokens)
    if len(parts) < 3:
        return respond('Need a question and ≥2 options.\n'
...
def handle_view_voters(ack, body, client):
    ...
    poll = store.get_poll(poll_id)
    if poll.anonymous:
        return  # button shouldn't have been rendered
    ...
    client.chat_postEphemeral(
        channel=poll.channel_id, user=body["user"]["id"],
        text="Voters:\n\n" + "\n\n".join(lines),
    )
...
def create_poll_api():
    ...
    return jsonify({"poll_id": poll.id, "channel": channel, "message_ts": ts,
                    "expires_at": poll.expires_at.isoformat() if poll.expires_at else None})


# Start expiry sweeper only when running as a real server (not under pytest)
